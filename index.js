...
app.get('/', (req, res) => { res.send('Welcome to Edurekas REST API with Node.js Tutorial!!');});
...
app.post('/api/books', (req, res)=> {
    const { error } = validateBook(req.body); // validate incoming req.body
    if (error){ res.status(400).send(error.details[0].message)//if error is true, status 404 send details error
    return;
    }
    const book = {
    id: books.length + 1,
